...
void pre_compute_lps(std::string *pattern, std::vector<ll> &prefix_suffix) {
    ll len {};
    ...
    while(i < pattern->length()) {
        if(pattern->at(i) == pattern->at(len)) {
            len++;
            prefix_suffix.at(i) = len;
            i++;
        } else {

            if(len != 0) {
                len = prefix_suffix.at(len - 1);
            } else {
...
void kmp(std::string *text, std::string *pattern, std::vector<ll> *result) {
    std::vector<ll> prefix_suffix(pattern->size());
...
/**
 * std::sort(v.begin(), v.end(), greater<T>()) sorts the elements in descending order.
*/
